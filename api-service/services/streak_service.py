from datetime import date


def _streak_message(
    new_streak: int,
    *,
    first: bool = False,
    consecutive: bool = False,
    used_grace: bool = False,
    broken: bool = False,
    already_completed: bool = False,
) -> str:
    if already_completed:
        return "Ya registraste este hábito hoy."
    if first:
        return "¡Primer día! El camino empieza aquí."
    if consecutive:
        return f"¡{new_streak} días seguidos! Sigue así 💪"
    if used_grace:
        return f"Usaste un día de gracia. Racha: {new_streak} días."
    if broken:
        return "Nuevo comienzo. Cada día es una oportunidad 🌱"
    return f"¡{new_streak} días seguidos! Sigue así 💪"


def calculate_streak_update(streak: dict, completion_date: date) -> dict:
    """Calculate streak state after a habit completion.

    streak keys: current_streak, longest_streak, last_completion,
    grace_days_used, grace_days_allowed
    """
    if streak["last_completion"] is None:
        new_streak = 1
        return {
            **streak,
            "current_streak": new_streak,
            "longest_streak": max(new_streak, streak["longest_streak"]),
            "last_completion": completion_date,
            "broken": False,
            "used_grace_day": False,
            "message": _streak_message(new_streak, first=True),
        }

    gap = (completion_date - streak["last_completion"]).days

    if gap <= 0:
        return {
            **streak,
            "broken": False,
            "used_grace_day": False,
            "message": _streak_message(
                streak["current_streak"],
                already_completed=True,
            ),
        }

    if gap == 1:
        new_streak = streak["current_streak"] + 1
        used_grace = False
        broken = False
        grace_days_used = streak["grace_days_used"]
        message = _streak_message(new_streak, consecutive=True)
    elif gap == 2 and streak["grace_days_used"] < streak["grace_days_allowed"]:
        new_streak = streak["current_streak"] + 1
        used_grace = True
        broken = False
        grace_days_used = streak["grace_days_used"] + 1
        message = _streak_message(new_streak, used_grace=True)
    else:
        new_streak = 1
        used_grace = False
        broken = True
        grace_days_used = 0 if gap > 2 else streak["grace_days_used"]
        message = _streak_message(new_streak, broken=True)

    return {
        **streak,
        "current_streak": new_streak,
        "longest_streak": max(new_streak, streak["longest_streak"]),
        "grace_days_used": grace_days_used,
        "last_completion": completion_date,
        "broken": broken,
        "used_grace_day": used_grace,
        "message": message,
    }
