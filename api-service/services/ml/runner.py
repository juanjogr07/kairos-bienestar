"""
ML runner — orquestador diario de modelos (KAI-51).

Responsibilities:
- Fetch usage_events from Supabase for each user (last 7 days)
- Extract features via features.py
- Score via isolation_forest.score_user()
- Upsert result to ml_results (one row per user per day per model_type)
